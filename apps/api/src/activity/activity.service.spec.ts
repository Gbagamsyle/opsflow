...
describe('ActivityService', () => {
  ...
  it('records an activity with actor and metadata', async () => {
    ...
    prisma.activityLog.create.mockResolvedValue({ id: 'activity-1', ...activity });

    await expect(service.record(activity)).resolves.toMatchObject({ id: 'activity-1' });
  ...
  it('only returns activity for organization members', async () => {
    ...
    await expect(service.findForUser('org-1', 'user-1')).rejects.toThrow('Organization not found');
