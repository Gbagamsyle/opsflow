...
export default function ProjectWorkspacePage() {
  ...
  useEffect(() => {
    ...
    async function loadWorkspace() {
      try {
        const current = await apiRequest<{ organization: { id: string } }>("/organizations/current", getToken);
  ...
  if (!isLoaded || loading) return <div className={styles.loadingState}>Loading project workspace...</div>;
    ...
    <section className={styles.projectWorkspace}>
      ...
      {project && (
        <header className={styles.projectWorkspaceHeader}>
          ...
          <button type="button" className={styles.primaryAction}><Plus size={14} /> Add task</button>
        </header>
      )}
